...
# === TOPIC PRINCIPAL POUR LES DONNÉES COMPLÈTES OBD-II ===
# IMPORTANT: On s'abonne à wincan/device# pour capturer tous les devices
# Éviter wincan/# qui inclurait aussi les topics individuels (EngineCoolantTemp, VehicleSpeed, etc.)
MQTT_TOPIC_PATTERN = "wincan/device#"  # Pattern MQTT pour tous les devices (device1, device2, device3, etc.)
...
def on_message(client, userdata, msg):
    ...
    try:
        ...
        # ✅ FILTRAGE STRICT: Accepter UNIQUEMENT les topics wincan/deviceX
        # Rejeter les topics individuels comme wincan/EngineCoolantTemp
        if not topic.startswith("wincan/device"):
            print(f"⚠️  TOPIC IGNORÉ: {topic}")
            print(f"   ℹ️  Seuls les topics 'wincan/device1', 'wincan/device2', etc. sont acceptés")
            print(f"   ℹ️  Les topics individuels (wincan/EngineCoolantTemp, etc.) sont ignorés")
            print("="*70 + "\n")
            return
        ...
        try:
            ...
            print(f"🔓 JSON parsé avec {len(data)} clés")
            ...
            # ✅ VALIDATION: Vérifier que c'est bien un JSON complet (plusieurs PIDs)
            # Si c'est un topic individuel, il n'aurait qu'une seule clé
            if len(data) < 5:
                print(f"⚠️  MESSAGE IGNORÉ: JSON trop petit ({len(data)} clés)")
                print(f"   ℹ️  Format attendu: JSON avec plusieurs PIDs (format complet)")
                print("="*70 + "\n")
                return
            
            # ✅ Mettre à jour le timestamp et l'état SEULEMENT si données valides
            ...
            for pid_key, value in data.items():
                if pid_key in PID_TO_COLUMN_MAPPING:
                    ...
                    if field_name in ["vehicle_speed", "rpm", "coolant_temperature", "engine_load"]:
                        print(f"   🔵 {pid_key} → {field_name} = {value}")
